...
nav = st.sidebar.radio(
    "섹션 이동",
    ["홈", "물리학", "화학", "생명과학", "지구과학", "비교", "진로", "MBTI 추천", "미니퀴즈", "FAQ"],
    captions=[
        "소개 & 하이라이트",
        "⚛️ 힘·파동·우주",
        "🧪 물질·반응·산업",
...
if nav == "MBTI 추천":
    ...
    st.markdown("<div class='card'>학생의 성향(MBTI)과 과제 선호를 바탕으로 네 과목 중 어울리는 과목을 추천해요! 🧭✨</div>", unsafe_allow_html=True)
    ...
    with colL:
        ...
        mbti = st.selectbox("🧩 MBTI를 선택하세요", mbti_types, index=5)
        st.caption("※ 재미/참고용 추천이에요. 실제 흥미와 목표가 더 중요! ✨")
    with colR:
        st.markdown("**선호 옵션(선택)** 🎛️")
        like_math = st.slider("수식·논리 좋아함", 0, 10, 5)
        like_lab = st.slider("실험·장비 좋아함", 0, 10, 5)
        like_bio = st.slider("생명·의학 관심", 0, 10, 5)
        like_out = st.slider("야외 관측/지도 좋아함", 0, 10, 5)

    # 기본 MBTI → 과목 매핑 (사용자 동의 완료)
    base_map = {
        # 분석가/전략가
        "INTJ": "물리학", "INTP": "화학", "ENTJ": "물리학", "ENTP": "물리학",
        # 외교형
        "INFJ": "생명과학", "INFP": "생명과학", "ENFJ": "생명과학", "ENFP": "생명과학",
        # 관리자형
        "ISTJ": "물리학", "ISFJ": "화학", "ESTJ": "화학", "ESFJ": "화학",
        # 탐험가형
        "ISTP": "지구과학", "ISFP": "지구과학", "ESTP": "지구과학", "ESFP": "지구과학",
    }

    # 가중치 점수 계산
    score = {"물리학":0, "화학":0, "생명과학":0, "지구과학":0}
    score[base_map[mbti]] += 2  # MBTI 기본 추천 가중치
    score["물리학"] += like_math
    score["화학"] += like_lab
    score["생명과학"] += like_bio
    score["지구과학"] += like_out
    ...
    st.success(f"✨ 추천 과목: **{best}**  — 점수 {score}")

    # 추천 이유 및 바로가기
    reasons = {
        "물리학": "논리·수학 기반 문제해결을 즐기며, 모델링/시뮬·엔지니어링 관심과 잘 맞아요 ⚙️📐",
        "화학": "실험 설계와 데이터 해석을 좋아하고, 신소재·반도체·제약 분야로 확장하기 좋아요 🧪🧫",
        "생명과학": "의생명/바이오 관심이 크고, 관찰·기록·팀 프로젝트에 강점이 있어요 🧬🔬",
        "지구과학": "야외 관측/지도·시계열 데이터를 좋아하고, 기상·천문·환경에 흥미가 커요 🌦️🔭",
    }
    st.info(reasons[best])

    # 바로 살펴보기 버튼
    cols = st.columns(4)
    dest = {"물리학":"물리학", "화학":"화학", "생명과학":"생명과학", "지구과학":"지구과학"}[best]
    with cols[0]:
        if st.button("⚛️ 물리학 보기", use_container_width=True):
            st.session_state["_goto"] = "물리학"
    with cols[1]:
        if st.button("🧪 화학 보기", use_container_width=True):
            st.session_state["_goto"] = "화학"
    with cols[2]:
        if st.button("🧬 생명과학 보기", use_container_width=True):
            st.session_state["_goto"] = "생명과학"
    with cols[3]:
        if st.button("🌍 지구과학 보기", use_container_width=True):
            st.session_state["_goto"] = "지구과학"

    # 세션 이동 처리(간단 구현)
    if st.session_state.get("_goto"):
        st.toast(f"{st.session_state['_goto']} 탭으로 이동하려면 사이드바에서 선택하세요 ✨")
